...
export default function Navbar({darkMode,toggleDarkMode}){
    ...
    return (
        <div className="flex fixed w-full z-50 mt-4 justify-center">
            <motion.nav
            initial={{y:-10}}
            animate={{y:0}}
                ...
                <div className="flex items-center justify-between w-full space-x-6 lg:space-x-8">
                    ...
                    <div className="hidden md:flex items-center space-x-6">
                        {navItems.map((item)=>(
                            ...
                            >
                                <motion.span
                                className={`font-medium transition-colors duration-300 ${isActive===item.name.toLowerCase()?colors.textActive:`${colors.textSecondary} hover:text-orange-500`}`}
                                whileHover={{scale:1.05}}
                                whileTap={{scale:0.95}}>
                                    {item.name}
                                </motion.span>
                    ...
                    <div className="flex md:hidden items-center space-x-4 px-2">
                        <motion.button
                        whileTap={{scale:0.9}}
                        onClick={()=>setIsMenuOpen(!isMenuOpen)}
                        className={`p-2 rounded-lg ${darkMode?'bg-gray-700':'bg-gray-200'}`}>
                            {isMenuOpen?(<X className={`w-5 h-5 ${darkMode?'text-white':'text-gray-700'}`}/>):(<Menu className={`w-5 h-5 ${darkMode?'text-white': 'text-gray-700'}`}></Menu>)}
                        </motion.button>
                    </div>
                </div>
                {isMenuOpen && (
                   ...
                   className={`absolute top-full left-0 right-0 mt-2 md:hidden ${darkMode ? 'bg-gray-900/95':'bg-white/95'} backdrop-blur-lg rounded-xl shadow-lg border ${darkMode?'border-gray-700':'border-gray-200'}`}>
                        ...
                        {navItems.map((item)=>(
                            ...
                            className="block">
                                <motion.div 
                                whileHover={{x:5}}
                                className={`py-3 px-4 rounded-lg text-center ${isActive === item.name.toLowerCase()?darkMode?'bg-gray-800':'bg-orange-50':''
                                }`}>
                                    <span className={`font-medium ${isActive === item.name.toLowerCase()?colors.textActive: colors.textSecondary}`}>
                                        {item.name}
                                    </span>
                                </motion.div>
                            </a>
